...
/** @defgroup pqd_motorpowermeasurement PQD Motor Power Measurement
  ...
  *
  * the PQD Motor Power Measurment component uses @f$I_d@f$, @f$I_q@f$, @f$V_d@f$ and @f$V_q@f$
  ...
  * instatenous power values. The instantaneous values are then used to compute an average 
  * power value that is stored. These computations are done with interger operations and the
  * average value is store as an integer, in s16 digit format (s16A x s16V unit). 
  * 
  * The PQD Motor Power Measurment component provides an interface, PQD_GetAvrgElMotorPowerW()
  * that converts the integer s16 digit average power into a floating point value expressed in 
  * Watts.
  *
  * @{
  */
...
/**
  ...
  *        of the DQ current and voltages. 
  * 
  *  This method should be called with periodicity (Medium Frequency Task). It computes an 
  ...
  * 
  * Computations are done on s16A and s16V integer values. The average motor power value is 
...
/**
  * @brief  Clears the the s16 digit average motor power value stored in the handle.
  * 
  * This function should be called before each motor restart.
  * 
  * @param power Handle on the related PQD Motor Power Measurement component instance.
  */
...
/**
  ...
  * 
  * This function converts the s16 digit average motor power value stored in the handle
