/**
  ...
  *          microcontrollers and implements the sensing using one, two or three
  *          indipendent ADC with injected channels.
...
__weak void BADC_SetSamplingPoint(Bemf_ADC_Handle_t *pHandle, PWMC_Handle_t *pHandlePWMC, SpeednTorqCtrl_Handle_t *pHandleSTC )
{
  ...
  LL_GPIO_ResetOutputPin( pHandle->pParams_str->bemf_divider_port, pHandle->pParams_str->bemf_divider_pin );
...
{
  uint32_t LfTimPeriod = ( uint32_t )((pHandle->PseudoPeriodConv * SPEED_UNIT) / (hAvrMecSpeedUnit * pHandle->hForcedDirection * U_RPM));  
