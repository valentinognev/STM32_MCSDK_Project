...
typedef struct
{
  ...
  volatile uint16_t *ADCDataReg1[6];   /*!< Stores ADC read value's address for the 6 sectors. Specific to F0XX and G0XX. */
  volatile uint16_t *ADCDataReg2[6];   /*!< Stores ADC read value's address for the 6 sectors. Specific to F0XX and G0XX. */
  uint16_t hDeadTime;                  /*!< Dead time in number of TIM clock
                                            cycles. Specific to F0XX and G0XX.
                                            If CHxN are enabled, it must
  ...
  uint16_t hTbefore;                   /* Sampling time express in number of TIM clocks.*/
  uint16_t Tsampling;                  /* Sampling time express in number of TIM clocks.*/
  uint16_t Tcase2;                     /* Sampling time express in number of TIM clocks.*/
  uint16_t Tcase3;                     /* Sampling time express in number of TIM clocks.*/
  uint8_t  RepetitionCounter;          /* Expresses the number of PWM
                                          periods to be elapsed before compare
                                          registers are updated again. In
                                          particular:
                                          RepetitionCounter= (2* #PWM periods)-1*/
  uint8_t  ADCScandir[6];              /*!< Stores ADC scan direction for the 6 sectors. Specific to F0XX and G0XX.*/
...
typedef struct
{
  ...
  bool ADCRegularLocked;        /*!< This flag is set when regular conversions are locked. Specific to F0XX and G0XX. */
...
/*
 * Contains the OVERCURRENT Break event interrupt.
 */
void *R3_1_OVERCURRENT_IRQHandler( PWMC_R3_1_Handle_t * pHandle );

/*
 * Contains the OVERVOLTAGE Break event interrupt.
 */
