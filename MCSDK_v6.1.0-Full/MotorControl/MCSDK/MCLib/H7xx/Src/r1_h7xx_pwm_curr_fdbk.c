/**
  ...
  *           + initializes MCU peripheral for 1 shunt topology and H7 family
...
/**
 * @defgroup R1_H7XX_pwm_curr_fdbk H7 R1 1 ADC PWM & Current Feedback
 *
 * @brief STM32H7, 1-Shunt, 1 ADC, PWM & Current Feedback implementation
 *
 * This component is used in applications based on an STM32H7 MCU
 ...
 *         in ICS configuration using STM32H7x High Density
...
__weak void R1F30X_CurrentReadingCalibration( PWMC_Handle_t * pHdl )
{
  ...
  hMaxPeriodsNumber = ( NB_CONVERSIONS + 1u ) * ( ( uint16_t )( pHandle->pParams_str->RepetitionCounter ) + 1u );

  LL_TIM_ClearFlag_CC1( TIMx );
  hCalibrationPeriodCounter = 0u;
  while ( pHandle->Index < NB_CONVERSIONS )
  {
...
/**
  ...
  * @retval none
  */
__weak void R1F30X_TurnOnLowSides( PWMC_Handle_t * pHdl, uint32_t ticks )
{
  PWMC_R1_F3_Handle_t * pHandle = ( PWMC_R1_F3_Handle_t * )pHdl;
  TIM_TypeDef * TIMx = pHandle->pParams_str->TIMx;
